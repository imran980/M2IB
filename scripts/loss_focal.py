...
class FocalLoss_survival(nn.Module):
    r"""
    ...
    """
    def __init__(self, class_num, alpha=None, gamma=2, size_average=True):
        super(FocalLoss_survival, self).__init__()
    ...
    def forward(self, inputs, targets, survived_time):
        N = inputs.size(0)
        C = inputs.size(1)
        P = F.softmax(inputs)
        ...
        #
        i = age-i
        para_p = np.abs((np.exp(i) - np.exp(-i)) / (np.exp(i) + np.exp(-i)))

...
class IBAInterpreter:
    def __init__(self, model, estim: Estimator, beta, steps=10, lr=1, batch_size=10, progbar=False, dim_model=768):
    ...
    def _run_vision_training(self, text_t, image_t, **kwargs):
        ...
        image_repr = self.model.get_text_features(text_t)
        print("_run_vision_training text_repr------------------------:", image_repr.shape)
       # _, attended_image = self.sequential(self.model.get_image_features(image_t), other_repr=image_repr)
        _, attended_image = self.sequential((self.model.get_image_features(image_t), image_repr))
